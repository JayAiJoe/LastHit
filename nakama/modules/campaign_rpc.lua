...
local function get_campaign_id(_,_)
    local matches = nakama.match_list()
    local current_match = matches[1]

    if current_match == nil then
        return nakama.match_create("campaign_control", {})
    else
...
-- @return "1"
local function remove_player_name(_, payload)
...
local function register_player_name(_, payload)
    local names = _get_name_collection().names
...
nakama.register_rpc(get_campaign_id, "get_campaign_id")
nakama.register_rpc(register_player_name, "register_player_name")
nakama.register_rpc(remove_player_name, "remove_player_name")
